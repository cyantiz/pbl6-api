...
CREATE TABLE `post` (
    ...
    `status` ENUM('PENDING', 'DELETED', 'ACTIVE', 'DENIED') NOT NULL DEFAULT 'PENDING',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `change_request` (
    ...
    `postId` INTEGER NOT NULL,
    `reason` VARCHAR(191) NOT NULL,
    `targetStatus` ENUM('PENDING', 'DELETED', 'ACTIVE', 'DENIED') NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
-- AddForeignKey
ALTER TABLE `visit` ADD CONSTRAINT `visit_postId_fkey` FOREIGN KEY (`postId`) REFERENCES `post`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `change_request` ADD CONSTRAINT `change_request_postId_fkey` FOREIGN KEY (`postId`) REFERENCES `post`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
